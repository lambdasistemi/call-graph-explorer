...
-- | node IDs, plus their module parents and
-- | connecting edges.
...
-- | List all module names in the graph.
modules :: Graph -> Array ModuleName
modules graph =
  Array.nub
    $ map _.module_
    $ Array.fromFoldable
...
-- | Get all node IDs belonging to a module.
nodesByModule :: ModuleName -> Graph -> Set NodeId
nodesByModule modName graph =
  foldl
    ( \acc n ->
        if n.module_ == modName then
          Set.insert n.id acc
        else acc
    )
    Set.empty
